...
def draw_H_and_E(citys, H_path, energys):
    ...
    for (from_, to_) in H_path:
        ...
        ax1.annotate(text=to_, xy=citys[to_], xytext=(-8, -4), textcoords='offset points', fontsize=20)
    ax1.axis('equal')
    ax1.grid()
    # 绘制能量趋势图
...
if __name__ == '__main__':
    ...
    citys = np.loadtxt('./p01_xy.txt', dtype=np.double, delimiter=' ')
    #distance = get_distance(citys)
    distance = np.loadtxt('./p01_d.txt', delimiter='        ')
    print(distance)
    N = len(citys)
    # 设置初始值
    A = N * N
    ...
    for n in range(num_iter):
        ...
        if len(np.unique(route)) == N:
            ...
            if dis < best_distance:
                ...
                [print(v, end=',' if i < len(best_route) - 1 else '\n') for i, v in enumerate(best_route)]
    if len(H_path) > 0:
